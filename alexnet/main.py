...
parser.add_argument('--d', '--dataset', dest='dataset',
                    choices = ['imagenet', 'cifar100'], default='imagenet',
                    help='Choose between the ImageNet or CIFAR100 datasets.')
...
parser.add_argument('--b-lr', '--backward-learning-rate', default=0.01, type=float,
                    metavar='BLR', help='initial learning rate for bacward weights', dest='b_lr')
...
parser.add_argument('--t', '--train-mode', dest='train_mode',
                    choices = ['BP', 'DFA', 'DKP'], default='BP',
                    help='Choose between backpropagation (BP), Direct '
                    'Feedback Alignment (DFA), or Direct Kolen Pollack(DKP).')
...
################################################################################
    model = alexnet(args.train_mode, args.batch_size)
################################################################################
    ...
    if args.train_mode in ['BP', 'DFA']:
        optimizer = torch.optim.SGD(model.parameters(), args.lr,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
    elif args.train_mode in ['DKP']:
        init_train(train_loader, model, criterion, args)
        forward_params = []
        backward_params = []
        for name, param in model.named_parameters():
            # print(name, type(param.data), param.size(), param.is_leaf, param.requires_grad)
            if "backward_weights" in name:
                # print(name, type(param.data), param.size(), param.is_leaf, param.requires_grad)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        one_hot = torch.nn.functional.one_hot(target, 100).float()
        output = model(images, one_hot)
...
def init_train(train_loader, model, criterion, args):
    print("Passing through network...")
    # switch to train mode
    model.train()
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    b_lr = args.b_lr * (0.1 ** (epoch // 30))
    if args.train_mode == 'BP':
